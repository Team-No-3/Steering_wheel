...
def build_generator(z_dim):
    ...
    # 256 * 64 * 64 번 선형회귀 결과를 64, 64, 256 의 3차원 배열로 변환
    ...
    #  Conv2DTranspose 객체를 이용해서 64,64,512 배열을 128,128,512 
...
def build_discriminator(img_shape):
    ...
    # CNN 필터 학습 256,256,3 배열을 256x256x32 배열로 변환
    model.add(
        Conv2D(32,
               kernel_size=3,
               strides=1,
    ...
    # Leaky ReLU activation
    model.add(LeakyReLU(alpha=0.01))
    # CNN 필터 학습 256x256x3 배열을 128x128x32 배열로 변환
    model.add(
        Conv2D(32,
               kernel_size=3,
               strides=2,
    ...
    # Leaky ReLU activation
    model.add(LeakyReLU(alpha=0.01))
    # Convolutional layer, from 128x128x32 into 128x128x64 tensor
    model.add(
        Conv2D(64,
               kernel_size=3,
               strides=1,
    ...
    # Convolutional layer, from 128x128x64 into 64x64x64 tensor
    model.add(
        Conv2D(64,
               kernel_size=3,
               strides=2,
    ...
    # Convolutional layer, from 64x64x64 tensor into 64x64x128 tensor
    model.add(
        Conv2D(128,
               kernel_size=3,
               strides=1,
    ...
    # Leaky ReLU activation
    model.add(LeakyReLU(alpha=0.01))
    # Convolutional layer, from 64x64x128 tensor into 32x32x128 tensor
    model.add(
        Conv2D(128,
               kernel_size=3,
               strides=2,
    ...
    # Convolutional layer, from 32x32x128 tensor into 32x32x256 tensor
    model.add(
        Conv2D(256,
               kernel_size=3,
               strides=1,
    ...
    # Convolutional layer, from 32x32x256 tensor into 32x32x512 tensor
    model.add(
        Conv2D(512,
               kernel_size=3,
               strides=1,
